...
export default function CreateQuotation() {
  ...
  const [timeline, setTimeline] = useState([
    { id: Date.now(), phase: 'Phase 1', title: 'Design & Setup', description: '', duration: '1-2 days' }
  ]);
  ...
  const [paymentSchedule, setPaymentSchedule] = useState([
    { id: Date.now(), title: 'Advance Payment', percentage: 20, condition: 'Required to commence development.', amount: 0 }
  ]);
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      // We need to temporarily ensure the header and footer are visible for html2canvas
      // and take a snapshot of them.
      const headerCanvas = await html2canvas(headerElem, { scale: 4, useCORS: true, logging: false });
      const footerCanvas = await html2canvas(footerElem, { scale: 4, useCORS: true, logging: false });
      
      const headerImgData = headerCanvas.toDataURL('image/png');
      const footerImgData = footerCanvas.toDataURL('image/png');

      // A4 width is 210mm. Calculate proportional heights.
      // Header is ~147px (115 + 32). If 800px = 210mm, 147px = 38.5875mm.
      const headerHeight = 45; // added top padding
      // Footer is ~50px. 50px = ~13mm.
      const footerHeight = 15;

      const opt = {
        margin:       [headerHeight + 2, 0, footerHeight + 2, 0], // Top, Left, Bottom, Right
      ...
      // Generate the PDF only from the main content. html2pdf handles the smart page breaks.
      ...
      await worker.get('pdf').then((pdf) => {
        ...
        for (let i = 1; i <= totalPages; i++) {
          ...
          // Stamp the Header SVG image at the top of the page
          // 800px width = 210mm. 147px height = (147/800)*210 = 38.5875mm.
          pdf.addImage(headerImgData, 'PNG', 0, 0, 210, 38.5875);
          
          // Stamp the Footer SVG image at the bottom of the page
          // 50px height = (50/800)*210 = 13.125mm.
    ...
    <div className="flex h-screen overflow-hidden font-sans bg-slate-50 print:bg-white print:h-auto print:overflow-visible">
      ...
      <div className="flex-none w-[550px] bg-white overflow-y-auto border-r border-slate-200 shadow-[2px_0_10px_rgba(0,0,0,0.05)] flex flex-col print:hidden">
        <div className="p-5 bg-[#0E5E64] text-white sticky top-0 z-10 flex justify-between items-center">
          <h2 className="m-0 text-xl font-semibold">Create Quotation</h2>
        ...
        <div className="w-[800px] min-h-[1131px] bg-[#FFFFFF] shadow-[0_10px_40px_rgba(0,0,0,0.1)] relative font-sans text-[#1E293B] box-border print:w-full print:min-h-0 print:shadow-none print:m-0 print:p-0" ref={previewRef} id="quotation-preview">
          ...
          <div id="pdf-header" className="w-full bg-[#FFFFFF] pt-8">
            <div className="relative w-full h-[115px] bg-[#0E5E64] flex items-center overflow-hidden print-color-adjust-exact">
              {/* The White Base for Logo with Straight Yellow Accent Edge */}
