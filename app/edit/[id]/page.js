...
import styles from '../../create/invoice.module.css';
...
export default function EditInvoice({ params }) {
  ...
  useEffect(() => {
    async function fetchInvoice() {
      try {
        ...
        setFormData({
          salespersonName: data.salespersonName || 'Manthan Vaghasiya\nFounder - Webiox Digital Solution',
          ...
          upiNumber: data.paymentDetails?.upiNumber || '+91 9664736245',
          upiId: data.paymentDetails?.upiId || 'manthanvaghasiya@oksbi',
          email: data.paymentDetails?.email || 'manthanvaghasiya@webiox.tech',
        });
        ...
        if (initialMilestones.length > 0) {
          setMilestones(initialMilestones.map((m, index) => ({
            id: m._id || m.id || index,
            title: m.title || '',
            description: m.description || '',
            amount: m.amount || 0,
            isPaid: m.isPaid || false,
            paidDate: m.paidDate ? new Date(m.paidDate).toISOString().split('T')[0] : ''
          })));
        } else {
          setMilestones([
            { id: Date.now(), title: '', description: '', amount: '', isPaid: false, paidDate: null }
          ]);
        }

        let qr = data.images?.qrCodeUrl || '/WhatsApp_Image.png';
        if (qr.includes('WhatsApp')) qr = '/WhatsApp_Image.png';

        setImages({
          logo: data.images?.logoUrl || '/WEBIOX icon + name.png',
          qrCode: qr,
          signature: data.images?.signatureUrl || '/IMG_20251109_132305-removebg-preview.png',
        });
      } catch (error) {
        console.error(error);
  ...
  // Auto-download PDF logic
  useEffect(() => {
    if (!isLoading && typeof window !== 'undefined') {
      const urlParams = new URLSearchParams(window.location.search);
      if (urlParams.get('autoDownload') === 'true') {
        // Wait briefly for images/fonts to fully render
        setTimeout(() => {
  ...
  if (isLoading) {
    return <div style={{ padding: '40px', textAlign: 'center' }}>Loading Invoice Data...</div>;
  }
